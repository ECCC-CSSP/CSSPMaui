...
public partial class CSSPCreateGzFileService : ControllerBase, ICSSPCreateGzFileService
{
    ...
    {
        string FunctionName = $"async Task<bool> CreateWebMWQMSamples2021_2060GzFileAsync(SubsectorTVItemID: { SubsectorTVItemID })";
        ...
        try
        {
            ...
            if (!await FillMWQMSampleModelList2021_2060Async(webMWQMSamples1980_2020.MWQMSampleModelList, TVItemSubsector)) return await Task.FromResult(false);

            if (Local)
            {
                if (!await StoreLocalAsync<WebMWQMSamples1980_2020>(webMWQMSamples1980_2020, $"{ WebTypeEnum.WebMWQMSamples2021_2060 }_{ SubsectorTVItemID }.gz")) return await Task.FromResult(false);
            }
            else
            {
                if (!await StoreAsync<WebMWQMSamples1980_2020>(webMWQMSamples1980_2020, $"{ WebTypeEnum.WebMWQMSamples2021_2060 }_{ SubsectorTVItemID }.gz")) return await Task.FromResult(false);
            }
        }
        catch (Exception ex)
        {
