...
public partial class Startup : IStartup
{
    public async Task<bool> Generate()
    {
        ...
        string? IEnumsGeneratedText = Configuration["IEnumsGenerated"];
        string? EnumsGeneratedText = Configuration["EnumsGenerated"];

        if (IEnumsGeneratedText == null)
        {
            Console.WriteLine($"{nameof(IEnumsGeneratedText)} is null");
            return await Task.FromResult(false);
        }

        if (EnumsGeneratedText == null)
        {
            Console.WriteLine($"{nameof(EnumsGeneratedText)} is null");
            return await Task.FromResult(false);
        }

        FileInfo fiInterface = new FileInfo(IEnumsGeneratedText);
        FileInfo fi = new FileInfo(EnumsGeneratedText);

        var importAssembly = typeof(AddressTypeEnum).Assembly;
        ...
        Console.WriteLine($"Created [{ fiInterface.FullName }] ...");
